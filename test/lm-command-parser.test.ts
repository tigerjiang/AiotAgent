...
import { parseLlmCommand } from "../src/parser/llm-command-parser";
...
describe("parseLlmCommand", async () => {
    ...
    if (result.success && result.command.intent === "start_cooking") {
        ...
        expect(result.command.deviceType).toBe("pellet_grill");
...
it("rejects an unsafe model-generated temparature", async () => {
    const provider = createProvider({
...
it("converts provider failures into MODE_ERROR", async () => {
